...
%What is a function? A na{\"i}ve yet enlightening perspective identifies category theory and type theory with distinct answers to this question. Without squinting too much, one may see category theory as a theory of functions abstracted as things which compose. With quite a bit more squinting we can, within type theory, locate a theory of functions abstracted as things which take things and make things.

%\par

%We can then see that internal-language/term-model relationships, between some sort of category and some variation of type theory, can give us, purely from the data of a category, a treatment of the objects of that category as if they were 'sets with elements'. This then permits a style of reasoning about those categories which is as intuitive as reasoning about sets with elements but which is derived, in a universal way, from the category itself as opposed to any analytic description of its objects and morphisms.

%\par

%The many internal language $\dashv$ term model/contextual category relationships are illuminated by this perspective. We see that relationships like:
%\begin{itemize}
%    \item the 2-equivalence
%        \[
%    	    \begin{tikzcd}[ampersand replacement=\&]
%    	    \mathsf{MLDTT}
%        	\arrow[r, "{\mathsf{Cont}}", shift left=1ex]
%    	    	\&
%    	        \mathsf{LocCartClosedCat}
%    	        \arrow[l,"{\mathsf{Lang}}", shift left=1ex]
%    	    \end{tikzcd}
%        \]
%between a 2-category of Martin-L{\"o}f dependent type theories, $\mathsf{MLDTT}$, and a %2-category of locally cartesian closed categories; or
%    \item the interpretability of homotopy type theory, $\mathsf{HoTT}$, in any $\left( %\infty, 1 \right)$-topos
%\end{itemize}
%give us, from the data of a category, a treatment of the objects of that category as if they were 'sets with elements' or 'spaces as $\omega$-groupoids'. This permits a style of reasoning about those categories which is as intuitive as sets with elements but which is derived, in a universal way, from the category itself as opposed to any analytic description of the objects and morphisms thereof.

% \par

% This blog post concerns a new work fitting into this constellation of relationships, Mike Shulman's:

% \begin{quotation}
%     "A Practical Type Theory for Symmetric Monoidal Categories"
% \end{quotation}

% Whereas in the well studied case of Martin-L\"{o}f dependent type theory, $\mathsf{MLDTT}$, or in the active program of homotopy type theory,  $\mathsf{HoTT}$, we began with a particular form of reasoning we wish to model.
% It is in some sense only after the fact that we have come to use these type theories to reason
% \emph{about} locally cartesian closed categories or $\left(\infty,1\right)$-toposes. Shulman's project with practical type theory
% is different. Here, we are to start with a domain and reverse engineer a syntax. We have a
% sort of category, symmetric monoidal categories, and wish to ask them
% to provide us with a type theory.
...
%
%For the type theory to be \emph{practical} Shulman suggests a few principles. First, for any symmetric monoidal category, not merely concrete and
%familiar ones, the type theory we get should permit us to leverage
%our intuition and experience with reasoning about sets with elements to reasona about 'sets with elements'. What we mean by this delightfully enigmatic phrase is that,
...
%*** every thing that is true in the tt in true in the language. This is what we need

%*** to be an internal language means that everything that is true in the category that it is provable in the language (i.e type theory)
